...
# Описание  
Выбранная архитектура - MVP. Она разделяет приложение на три компоненты:  
  
- Model - это бизнес-логика приложения, обладает знаниями о себе самой и не знает о презентерах и представлениях.  
- View - представление реализует отображение данных, которые получаются от модели.  
- Presenter - подписывается на события представления и по запросу изменяет модель (бизнес-логика).  
...
- setCategory - устанавливате категорию товара.  
