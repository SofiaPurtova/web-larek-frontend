...
В проекте используется паттерн MVP (Model-View-Presenter), он обеспечивает четкое разделение ответственностей между классами Model и View, при этом каждый класс выполняет свою определенную роль, не зная друг о друге:  
  
Слой данных Model - работа с загрузкой данных по API, сохранение и работа с данными полученными от пользователя.  
  
Слой отображения View - интерфейс для взаимодействия с пользователем, отлавливает и сообщает о произошедших событиях.  
...
- getListProductCard - получаем массив карточек с сервера.  
- postOrderLot - получаем ответ от сервера по сделанному заказу.  
...
### Класс FormModel управляет формой заказа.  
